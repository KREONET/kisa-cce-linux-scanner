...
- systemd inspection includes unit aliases, masks, template and type drop-ins, socket activation, and manager properties. When `systemctl show` rejects a bare service or socket template, absence is established only after successful queries find neither its unit-file definition nor any loaded instance; other cases remain collection errors. OpenSSH invocation checks enumerate loaded `ssh@*.service` and `sshd@*.service` instances, inspect those that are not inactive or failed, and retain custom `-f`, `-o`, or environment-expanded startup options as unresolved.
